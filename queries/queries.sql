...
WHERE grado.nombre = 'Grau en Enginyeria Informàtica (Pla 2015)';
...
SELECT d.nombre
FROM departamento d
LEFT JOIN asignatura a ON a.id = d.id
WHERE a.id IS NULL;
...
SELECT d.nombre AS departamento,
       COUNT(p.id) AS total
FROM departamento d
LEFT JOIN persona p ON p.id = d.id AND p.tipo = 'profesor'
GROUP BY d.id, d.nombre
ORDER BY total DESC;
...
SELECT g.nombre AS grado,
       COUNT(a.id) AS total
FROM grado g
LEFT JOIN asignatura a ON a.id = g.id
